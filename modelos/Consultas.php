<?php 
    ...
    Class Consultas {
        ...
        public function comprasfecha($fecha_inicio,$fecha_fin) {
            ...
            ON i.idusuario=u.idusuario WHERE DATE(i.fecha_hora)>='$fecha_inicio' 
        ...
        public function ventasfechacliente($fecha_inicio,$fecha_fin,$idcliente) {
            ...
            INNER JOIN usuario u ON v.idusuario=u.idusuario WHERE DATE(v.fecha_hora)>='$fecha_inicio' 
            AND DATE(v.fecha_hora)<='$fecha_fin' AND v.idcliente='$idcliente'";
            return ejecutarConsulta($sql);		
        }

        public function totalcomprahoy() {
            $sql="SELECT IFNULL(SUM(total_compra),0) as total_compra FROM ingreso 
            WHERE DATE(fecha_hora)=now()";
            return ejecutarConsulta($sql);
        }
